...
class Plot_NoLandmarks(Plot):
    ...
    def __call__(self, x: tensor, xhat: tensor, z:tensor,  epoch: int, n : int = 10, **kwargs):
        ...
        recreation_imgs = self.recreation_analysis(x_xyz= x_xyz_df, x_dist= x_dist_df, xhat_xyz =xhat_xyz_df,xhat_dist= xhat_dist_df, z =z, n_timepoints = n, current_epoch = epoch)
    ...
    def divide_tensor(self, tensor):
        ...
        Args:
            tensor (tensor): A tensor of shape(batch_size, 52), where the 52 tensor has the following
            structure: [joint_1_x, joint_1_y, joint_1_z, joint_1_dist_hip, ...,joint_n_x, joint_n_y, joint_n_z, joint_n_dist_hip]
