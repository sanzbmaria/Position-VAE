...
def center_hip(df):
    """
    Centers the others joints around the hip
...
def calculate_distance(df):
    ...
    parameters:
        df: the dataframe to calculate distance for

    returns:
        df: the dataframe with the calculated distances
    """
...
def check_hip_centered(df):
    ...
    Parameters:
    - df (pandas.DataFrame): A DataFrame containing joint data with a 'label' column.

    Returns:
    - bool: True if the hip joint has been centered correctly, False otherwise.
    """
